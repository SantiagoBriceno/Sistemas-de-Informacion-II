...
export const getViajeVehiculoAndConductor = async (req, res) => {
  try {
    const viaje = await service.getViajeVehiculoAndConductor()
    if (viaje) {
      res.send({
        status: 200,
        data: viaje
      ...
      res.status(404).send({
        status: 404,
        message: 'No se pudo obtener el viaje'
      })
    }
  } catch (e) {
    res.status(500).send({
      status: 'ERROR',
      message: 'OPS! ERROR AL OBTENER EL VIAJE'
    })
  }
}
